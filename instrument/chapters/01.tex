...
\lean{Measurement} does not begin with theory, and it does not begin with a device.
It begins with a witnessed difference that can be efficiently described.
Someone \lean{observes} something \lean{different}.

Before one speaks of laws, models, causes, or values, one must answer a more
primitive question: what is required for a witnessed distinction to admit a
next witnessed distinction? A scientific description is constrained not first
by what may be imagined, but by what may be observed, retained, and extended
without contradiction. Anything that cannot be witnessed and carried forward
in this way may belong to speculation or representation, but it does not yet
belong to fact.

For this reason, the basic object of this chapter is not a truth and not yet a
history. It is an admissible observed difference together with the possibility
of its continuation. Something happened, and it happened in a way that left a
recoverable distinction. That distinction may be coarse, local, and dependent
on the resolution of the observing process, but once admitted it constrains
what may consistently follow.

This shift in emphasis matters. Much confusion in the foundations of science
arises when mathematical convenience outruns operational continuation. A
description may be elegant, predictive, or analytically useful while still
introducing distinctions that no finite procedure could have carried from one
step to the next. Such structure may serve as representation, but it cannot
yet serve as physical constraint. The discipline adopted here is therefore
simple: no structure is allowed to do explanatory work unless it can be tied
to an admissible extension of an observed difference.

The consequence is that observation must be treated in its smallest usable
form. A beginning is not a continuum of values, but the possibility that
something was seen, that something was not seen, and that one witnessed
outcome may differ from another. Presence and absence are both informative.
A witnessed event constrains what may follow by what occurred. A verified
silence constrains what may follow by what did not occur. In either case, the
space of admissible continuation is narrowed.

At this point the central mathematical question appears. Given a present
difference, which next observations are admissible continuations of it? Not
every imaginable successor is allowed. Some proposed continuations preserve
coherence with what has already been witnessed; others do not. The passage
from a witnessed difference to an admissible next difference is the first
genuinely mathematical act of the chapter. It is an induction: a rule of
carry-forward. It introduces no external geometry and no law of motion. It
introduces only the requirement that the observed distinction be extendable
without undoing itself.

Once such an inductive carry-forward exists, counting becomes unavoidable.
If admissible distinctions may be extended step by step, then they may also be
enumerated step by step. Number is not added afterward as decoration. It is
forced by the repeated application of admissible continuation. A countable
structure therefore appears before any richer structure can be justified.

Only after this inductive mapping has been established do ledger, law, and
history become available. A ledger is a compiled record of admissible
continuations. A law is a local test on whether a proposed continuation slips
or holds. A history is a ledger read under such tests. In this way, the
chapter proceeds from witnessed difference to admissible continuation, from
admissible continuation to induction, from induction to counting, and only
then to the larger structures that science usually places first.

The aim of what follows is therefore modest and severe. We do not begin by
assuming continuity, geometry, or universal law. We begin by asking what must
be true if a finite observer is to witness a difference, preserve it, and
carry it forward by admissible induction into a coherent mathematical record.
...
\begin{phenom}{The Pythagoras--Planck Effect~\cite{euclid300bc,planck1900}}
\label{Finiteness}
