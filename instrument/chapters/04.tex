...
second counter advances each time the clock ticks. Each counter is a record of events:
wheels turning and clocks ticking. What the device preserves is not speed itself, but
the fact that certain changes occurred and that they occurred in a definite order.

At no point does the instrument measure a quantity in the abstract sense of assigning
a number to a property. It merely notes that another mark has been made. The familiar
dashboard display, showing a symbol such as “100 km/h,” is an interpretation layered on
top of this accumulated record. It is a convention applied after the fact, translating
patterns in counted events into a statement we have agreed to call a measurement.

Seen this way, the stability of any later conclusion rests entirely on what was actually
distinguished and recorded. Nothing enters the account except what was noticed,
marked, and re-marked as the process unfolded. The record does not support inferences
drawn from properties assumed in advance; it supports only those distinctions that
exist because the instrument, and not the theory, made them.

What, then, is it that gives rise to a new mark in the first place? If the record is the
permanent trace, there must be some circumstance that causes the trace to be made.
Before speaking about numbers, laws, or dynamics, we must isolate the minimal notion
that connects change in the world to change in the ledger. That notion is the event.
...
The same pattern appears whenever measurements are abstracted into theory.
Instruments produce finite or countable records; symbols are introduced to make
those records comparable; and only then are mathematical structures placed on
top of the record to support inference. What mathematics contributes is not new
observations, but disciplined rules for transforming and relating what has
...
Once multiple instruments are considered, an additional constraint enters. For
their records to be meaningfully compared, the symbols they produce must be
capable of being placed into a common account without contradiction. This
requirement does not concern how the symbols are interpreted, but whether the
distinctions they represent can coexist within a single history.
...
Let $\Sigma_i$ denote a finite set of distinguishable marks produced by an
instrument $i$. Each measurement made by the instrument corresponds to the
selection of one element of $\Sigma_i$. The alphabet $\Sigma_i$ specifies which
...
presenting the abstract number 100.  It is merely a symbol that we understand
to mean the magnitude of the velocity.
It is presenting one entry from a limited
...
\begin{definition}[Alphabet Enumeration]
    Let $\Sigma$ be a finite set of symbols. An \emph{enumeration} of $\Sigma$ is an
    injective function $\eta : \Sigma \to \mathbb{N}$. Two symbols
    $\sigma_1, \sigma_2 \in \Sigma$ are distinct if and only if
    $\eta(\sigma_1) \neq \eta(\sigma_2)$. No additional structure is implied or
    assumed.
\end{definition}

The existence of such an enumeration follows from finiteness alone and does not
depend on any ordering intrinsic to the symbols themselves. Different choices of
$\eta$ correspond to different labels for the same recorded distinctions and are
...
It is important to distinguish symbols from the events they indicate. A symbol
indicates which outcome an instrument has distinguished, but it does not identify
the particular act of distinction itself. What is preserved in the ledger is the
mark, not the circumstance that caused it.

The same symbol may therefore be produced by many distinct events. Repeated wheel
rotations, successive clock ticks, or separate radar returns may all leave
indistinguishable marks in the record, even though each arose from a different
physical situation. Symbols thus stand in a one-to-many relation with events: a
single mark may correspond to many possible underlying causes.

This asymmetry is not a defect of measurement but a structural feature of it. The
ledger necessarily collapses diverse circumstances into a finite set of
distinguishable outcomes. It is this collapse, rather than any ambiguity in the
events themselves, that gives rise to the phenomena explored next.
...
numerical evaluation but discrete choice. All higher mathematics is
...
subsection is to state explicitly the mathematical
...
provide the remaining axioms of measurement that enforce these same characteristics.

\section{Records}

By representing a record as a triple $(i, j, k)$ comprising an instrument label,
a symbol drawn from a finite, indexed alphabet, and a running successor count,
an observer cannot construct a phenomenon $p$ or a domain response $\psi$ that
is inconsistent with observation.  The mathematical objects carry, in their
very structure, the invariants an observer can later reason over independently
of any particular interpretation.

This representation is deliberately austere.  A record does not encode meaning,
cause, or explanation.  It encodes only what an observer can reliably store and
communicate: which instrument produced the observation, which distinguishable
symbol was produced, and how many times such a symbol has been observed.  Any
additional structure attributed to a record must arise from relations among
records, not from the record itself.

The necessity of this restriction follows from the nature of observation as
communication.  An observation is not a private event; it must be conveyed from
an instrument to an observer and, in principle, from one observer to another.
Whatever physical process underlies the measurement, the communicated outcome
must pass through a finite channel and be recoverable as a discrete description.
No observer can record, transmit, or compare an uncountable amount of
information in a single act of measurement.

This does not preclude refined instruments from operating on continuous models
or producing predictions that depend on arbitrarily fine structure.  It only
restricts what may be entered into the experimental ledger.  The ledger records
outcomes, not models.  Any claim about continuity, density, or smoothness must
be justified through refinement across records, not by assuming such structure
within a single record.

The following axiom makes this constraint explicit.  It formalizes the minimal
information content required of any instrument capable of contributing to the
experimental ledger, and it ensures that records may be compared, reordered,
and refined without introducing representational artifacts.
...
If the domain response exists, then a refinement operator can be constructed.
\end{proposition}
\begin{proofsketch}
Assume that a domain response $\psi(t)$ exists at refinement time $t$.
By Chapter~2, the existence of $\psi(t)$ permits the construction of a
Bayes predictor by composition with the induced inverse representational map,
\[
b(t) := \tilde\rho^* \circ \psi(t),
\]
where $\tilde\rho^*$ maps domain responses to symbols in the indexed
measurement alphabet.

Let $L_t$ be the experimental ledger at time $t$, and let
$r_t = (r_t.i, r_t.j, r_t.k)$ denote its final record.
Using the predicted symbol $b(t)$, construct a successor record by walking
the ledger: keep the instrument label fixed, insert the predicted symbol,
and set the count to one greater than the largest count previously appearing
in $L_t$ for the same instrument--symbol pair. Denote this record by
$r_{t+1}$.

Define the refined ledger by appending exactly this record,
\[
RL_t := [L_t, r_{t+1}].
\]
By construction, the ledger difference satisfies
\[
RL_t - L_t = \{ r_{t+1} \},
\]
which is a singleton event and therefore a valid event in the sense of
Chapter~1.

Define the refinement operator $R_t$ by $R_t(L_t) := RL_t$.
This operator appends exactly one new record, is determined uniquely by
$\psi(t)$, and produces an event that is recovered by subtracting the
original ledger from its refinement.

Therefore, whenever the domain response $\psi(t)$ exists, a refinement
operator exists at time $t$. This establishes the claimed sufficiency and
provides the operational content formerly attributed to the law of spline
sufficiency.
\end{proofsketch}
