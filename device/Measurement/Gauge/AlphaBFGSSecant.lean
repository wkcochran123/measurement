-- (Gauge phase) Relocated under `Measurement/Gauge/` — a self-contained BFGS-secant probe LEAF (defs used
-- only by its own #evals; nothing imports it), importing the physics one-way.
...
namespace Measurement.Gauge.AlphaBFGSSecant
open Measurement
...
def invAlphaAt (d : Q) : Nat :=
  (alphaFromSecondVariationAtDistance measuredT.toNat
    { numerator := d.1.toNat, denominator := d.2.toNat }).inverseScaledFloor (readoutScale)
...
end Measurement.Gauge.AlphaBFGSSecant
