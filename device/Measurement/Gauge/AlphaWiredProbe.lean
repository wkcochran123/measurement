-- (Gauge phase) Relocated under `Measurement/Gauge/` — a display/probe LEAF (its defs are consumed only by
-- its own `#eval`s; nothing external imports it), importing the physics one-way. The reporting layer, a directory.
...
namespace Measurement.Gauge.AlphaWiredProbe
...
def heartbeatCoeff : Int := (SelfApplication.heartbeatCoefficient : Int)  -- rank≡heartbeat/scale; sourced from SelfApplication (already imported), not a duplicate literal (audit #3)
...
open Measurement in
#eval s!"inv-alpha read at self-crossing orbit/echo: {(alphaFromSecondVariationAtDistance 5 (RationalDistance.mk orbitFix.natAbs echoFix.natAbs)).inverseScaledFloor (readoutScale)}"
open Measurement in
#eval s!"inv-alpha read at self-crossing echo/orbit: {(alphaFromSecondVariationAtDistance 5 (RationalDistance.mk echoFix.natAbs orbitFix.natAbs)).inverseScaledFloor (readoutScale)}"
end Measurement.Gauge.AlphaWiredProbe
