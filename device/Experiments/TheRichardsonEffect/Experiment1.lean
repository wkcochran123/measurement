...
namespace Experiments.TheRichardsonEffect

/-
Source description: The Richardson Effect
Claim ceiling: smoothShadowAnalogy.
The file records only resolution-dependent boundary measurement as a smooth-shadow analogy; it does not prove a fractal theorem.
-/
...
def modelKind : Experiments.Common.ModelKind :=
  Experiments.Common.ModelKind.projects

def defaultSetup : Setup :=
  { recorded := 2, accessible := 4, required := 1 }
...
end Experiments.TheRichardsonEffect
