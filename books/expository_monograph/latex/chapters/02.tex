...
The instrument can tell two things apart. That was the whole of the first rung, and it seemed like almost
...
The tower is still at its bottom. Nothing here is measured, nothing is weighed, nothing has a
...
is where number has to begin if it is to begin honestly, from below, without being handed down whole. So the
second step follows the same discipline as the first: add only what the construction earns, and watch precisely
...
actually has at this stage, and how much of it will have to be built, later, rung by rung, at a cost visible on
the page. So the step is deliberately meager. It gives the instrument the tally and the order and not one thing
more. The meagerness is the guarantee: what is not yet in the number cannot later be claimed from it for free.

\section{Number from a Difference Admitted}

Counting is one short step past telling apart: the instrument builds a count by keeping a
...
number will ever mean, in this construction, is going to be built out of exactly this and nothing richer. When, chapters
...
wearing a physical name. That is the discipline paying off in advance: because number did not arrive with more
than the tally in it, every quantity the instrument later reports can be traced back to what it is made of and
exactly what it cost. A number that came pre-loaded with meaning could never be audited
...
the first appearance, in the plainest possible setting, of a distinction that will run through the entire construction
...
the machine will have a name for each way. That naming is not earned here. But the seed is here, in the branch on
the order, and it is worth marking the spot so that
...
The floor first, because the discipline holds and the honesty lies in holding it. Nothing here has been proved.
Counting, and the ordering with its sign, are \emph{built} --- laid down as the machine's
...
descends from the first difference by definition, and the grade is the same as the previous floor's: what is
...
Now the reading, which is short because the construction is spare. A count, in this machine, is a how-many of
differences admitted, and that fixes what a count is \emph{of}. It is of the world, in exactly the sense the
first floor drew: the differences the machine admits are differences that are really there to be told, and the tally
...
the central line again, one rung higher than we left it: the count beneath is the world's, and the convention
