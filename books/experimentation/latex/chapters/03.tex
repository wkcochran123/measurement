...
orbit, a rotation --- where the record becomes a loop. And the loop is the instrument's most important new
capability, the one every later proof rests on: a closed path the instrument can carry a tally around, and
from which the tally can return \emph{changed}. That returning-changed is holonomy, and it is the seed of
every proof of a charged loop to come. The chapter introduces it gently --- first as the closed cycle of an
orbit, where the smooth-shadow ceiling debuts; then as the conserved tally of a rotation, the case where the
tally comes back unchanged; and finally, in Foucault's pendulum, as a residue a loop genuinely carries.
...
\begin{equation}
  \gamma(t + T) = \gamma(t).
\end{equation}
It is not equilibrium --- not a fixed point a body settles into --- but a stable failure to terminate, a loop
of length at least two that the record rereads forever. Because the path closes, the natural object for the
instrument is no longer a difference taken along a line but an integral taken around a loop --- the $\oint$
that will become this chapter's protagonist, a quantity accumulated all the way around and read where it
returns.

Kepler's three laws give the loop its shape, and they are best read as descriptions of the loop, not
...
\end{equation}
Each names a feature of the closed loop the instrument traverses --- its shape, its rate of sweep, its
size-and-period lock --- rather than a number to be solved for.
...
while the angle winds around. The whole closed cycle is fixed by two conserved invariants the loop carries:
the energy $E$, which sets how deep in the basin the motion sits, and the angular momentum $L$, which sets the
height of the centrifugal barrier. Whether the motion is bound or escapes is read off $E$; the shape of the
conic, off both together.
...
cycle comes back, a count condition it can check. But the smooth ellipse is something else, and it is the
smooth-shadow ceiling making its first appearance. The continuous curve $r = p/(1+e\cos\theta)$ is not a thing
the instrument holds; it is the smooth shadow that a closed discrete cycle of counts casts as the cycle is
imagined refined --- the shape the loop of counts approaches, named as a shadow and not derived as a fact.
...
The reading is Kepler's, recast: an orbit is a closed book that must be reread forever, sustained not by a
force pinning a body in place but by a reconciliation that never finishes. The loop is the instrument's new
home, and the integral around it the instrument's new tool.
...
boundary, angular momentum is conserved around a cycle. For a rigid body it takes the familiar form
\begin{equation}
...
angular momentum around unchanged unless something at the boundary turns it. The angular coordinate earns its
place by compression --- it folds two independent linear displacements into a single number, carrying half
the dimensional burden of the motion it describes.
...
momentum $\mathbf{L} = \mathbf{r} \times \mathbf{p}$. The instrument does not impose the conservation; it reads
it off the symmetry. The same principle returns in the next chapter as the general rule behind every conserved
quantity the book meets --- a symmetry breeds an invariant, and the invariant is a count the instrument can
carry around a loop unchanged.
...
instrument carries a tally around the loop and finds it unchanged; that unchangedness is the conservation,
and it is the calm before the more interesting case --- where what the instrument carries around the loop
does \emph{not} come back the same.
...
closed loop has left a residue, an angle the arrow has gained for going around. That leftover angle is the
\emph{holonomy} --- the residue a closed loop carries, read where the path returns.
...
equation describes that residue, it does not produce it.
...
This is the precise machinery the proved chapters will carry: where Foucault's loop encloses a smooth
curvature and returns a continuous angle, a discrete loop will enclose a quantized curvature and return a
residue of exactly $\pm 1$.
...
loop it is carried around will not let the swing's orientation close. The precession is the holonomy of that
loop, classical and gentle, visible to anyone who watches the pendulum for an afternoon.
...
that is conservation; sometimes it comes back with a residue, and that is holonomy. The smooth-shadow ceiling
