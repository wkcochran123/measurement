...
record say about what it did not directly see? The two halves --- the least principle, that the actual motion
extremizes an action, and the unseen, the stretches a record must reconstruct, the boundary that makes a
solution unique, the horizon past which prediction fails --- turn out to be one discipline. The instrument
fills the unseen with its least-cost guess, and the least principle is, in the end, the demand that the
unseen leave the smoothest trace it can. By the chapter's close the instrument has every capability the rest
of the book will use.
...
\begin{equation}
  \delta J = 0 .
\end{equation}
Read this as a description, not a calculation. It says: nudge the path a little, holding its endpoints fixed,
and to first order the cost does not change --- the instrument is sitting at the bottom of the cost, where
small re-routings buy nothing. The condition that a path sit there everywhere along its length has a shape,
...
path must have, the local signature of a global economy. Where it holds, the path is stationary; where it
fails, a cheaper re-routing exists and the path is not the one the instrument records.
...
cheaper one. The least principle is not a mystical preference for economy imposed on nature from outside; it
is the plain fact that the record the instrument lays down is the one that took the fewest distinctions to
write.
...
bridge between the cheapest sequence of counts and the differential equation is an analogy, named as one. The
reading is that the law of motion is not a force imposed from outside but an economy obeyed from within: the
path along which the instrument spends the fewest distinctions, described by the shape its frugality must
take.
...
instrument must reconstruct the unseen stretch, and it does so with its smoothest honest guess: the
minimal-curvature completion, the curve that bends as little as the endpoints allow. That curve is the one
whose fourth derivative vanishes,
\begin{equation}
  \Psi^{(4)} = 0,
\end{equation}
with neighboring patches glued so that the curve, its slope, and its bend are all continuous --- a $C^2$ fit,
the natural cubic spline through the anchors. And this smoothest guess is exactly the least-action extremal of
the previous section: smooth dynamics are precisely the histories that leave no trace, so two observers who
see nothing between the same endpoints reconstruct the very same cubic.

The honest floor is again a smooth-shadow analogy --- the discrete minimal-curvature reconstruction is the
shadow of the continuum extremal. The reading is that invisible motion is not unknown but uniquely
determined. The unseen stretch is the smoothest path the endpoints allow, which is the least-action path,
which is exactly why a motion that leaves no trace is repeatable at all: every honest instrument, asked to
fill the same silence, fills it the same way. The instrument does not guess wildly into the dark; it lays down
the one curve its economy demands, and labels it for what it is --- a reconstruction, the smoothest the
anchors permit, not a thing seen.
...
about the edge, not the interior --- discards the phantom and collapses the two solutions to one. The
instrument has fixed the interior, where the receiver is, from the boundary, the satellite ranges and the
surface constraint, without ever needing to scan the interior directly: it stands at the edge, reconciles
what the edge demands, and the interior follows uniquely. This is the same boundary reconciliation that will,
in the quantum and relativity parts, fix a record's order from its frontier without opening the record up;
here it appears in its plainest classical form, as trilateration.
...
values on the frontier, and the bulk is the boundary's unique harmonic continuation inward. This is the
precise form of fixing an interior from its edge, the same structure --- a frontier that determines a bulk ---
that the boundary reconciliations of the later parts carry.
...
uniqueness. What a record must commit to is its boundary; with it the interior is determined, and without it
the equations, however exact, leave a fork the instrument cannot close.
...
for it is chaos --- sensitive dependence, a butterfly's wing swaying a storm. But the instrument reads the
limit differently, and more honestly: it is not the dynamics amplifying a difference exponentially but the
instrument's own floor, grown until it swamps the signal. Two histories that differ only below the resolution
floor are, to the instrument, one history; as time runs forward, the gap between such histories grows, and at
some finite depth it exceeds what the record can hold. Count the admissible microhistories consistent with a
...
its divergence rate, what the instrument secures is its own horizon: a gap that starts at the floor, of size
$\epsilon_0$, and grows, where the motion is chaotic, exponentially,
\begin{equation}
...
far it can see, and it does not pretend to see past its own horizon.
...
yet in force, a label --- with which it has read all of mechanics off a finite record. Everything that
