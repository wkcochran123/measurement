...
The inverse-square fall-off --- the same $1/r^2$ that governs gravity and the electric force --- is usually
written down as a fact about force and left there. To the instrument it is not a fact about force at all but a
fact about geometry, and a simple one: a fixed budget of influence, spread over a frontier that grows as a
surface. This single effect closes Part II, and despite its brevity it is one of the instrument's deep ideas,
because it shows two of the great forces of nature to be the same piece of bookkeeping wearing two names. With
it, the reading of a field as the bookkeeping a record carries between events is complete.
...
--- a charge does not gain or lose its reach as you walk away from it, and neither does a mass. Whatever the
...
distance, but a conserved budget divided by a growing surface. The $1/r^2$ is the quotient of a constant over
$r^2$, and the $r^2$ is the frontier.
...
dimensions of the surface, one fewer than the dimensions of the space. The instrument does not put the exponent
in by hand; it reads it off how fast the frontier grows.
...
that grows as $r^2$. The instrument does not solve Gauss's law for the field; it counts the lines, notes that
the count is conserved, and reads the dilution off the growing surface.
...
read once as a flux through the frontier and once as a height a test point has climbed. Two readings, one
geometry --- and the potential the smoother of the two, which is why the bookkeeping of energy so often runs
through it rather than the field.
...
The $1/r^2$ is shared not by coincidence but because the geometry is shared; change the budget and you change
which force you are reading, but the law belongs to the surface, not to the force.
...
slope, no force. The interior is flat because the frontier put nothing there.
...
The inverse-square law and the holographic bound are the same frontier read two ways: once as what a budget
thins across, once as what a count cannot exceed. The instrument meets the frontier here, among the fields,
