...
% CHOICE (radar<->field, Kodo turn 27): the officer's EM instrument is framed abstractly here and through the
% coda arc as "the field that acts where it isn't" (gauge/holonomy), Doppler dropped per grade-off-code (no
% Part II chapter works Doppler). Ch30 (the finale) names its concrete form -- the radar gun / Doppler
% frequency-shift. This is by design: the three abstract witnesses (within/without/overhead) resolve to their
% concrete instruments (hall-sensor/radar/GPS) at the reveal, as overhead->GPS does. Accepted seam; Ch30 unchanged.
%% [operator: ratified]
