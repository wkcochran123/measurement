...
the instrument does the honest thing --- it names the effect and claims no more. This is the fourth and last
of its honest floors, the name-only floor, and the chapter marks its debut as carefully as the earlier
chapters marked the wall and the analogy. Where a finite count is a balance the instrument secures, a smooth
...
beneath it. Three effects mark the new floor and, with it, complete the instrument's set of four: the
semiconductor's bandgap, the superconductor's paired state, and the Meissner expulsion of flux.
...
Here the instrument meets the barest of its honest floors, the name-only floor, and the chapter marks its
debut as it earlier marked the wall and the analogy. The activation law above is the physics, offered as
physics; the finite model claims nothing but the label. It records that the effect is named --- ``the
semiconductor effect'' --- and models none of the band dynamics that produce it. The reading is that the
semiconductor is named, the activation law $n \propto e^{-E_g/2k_B T}$ is the physics, and the whole of the
finite model's claim is the name beneath it. This is not a retreat but an honesty: the instrument does not
pretend to a finite model of band structure it does not have.
...
into a single symmetric paired state carrying zero strain, $\mathrm{Strain}(\Psi_{\text{pair}}) = 0$, and an
...
effect and claiming nothing of its mechanism. The pair algebra that a finite construction might build around
such a state --- the bookkeeping of how paired carriers compose --- is not this chapter's subject, and the
chapter does not develop it. Here the superconductor is named, and the physics is left as physics. The reading
is that the superconductor is named, the gap and the paired state are physics, and the finite model is the
...
it. The reading is that the Meissner effect is named, the vanishing of the interior field is physics, and the
finite model is the name --- the third effect, and the last, to stand on the barest of the instrument's four
floors.

It is worth saying why the name-only floor appears here and not earlier. The instrument counted atoms in the
last chapter and balanced reactions in whole numbers, and it could do that because chemistry's facts are, at
...
leaves to physics. For every effect in every chapter to come, the instrument will stand on exactly one of
...
order is a name the count settles into. Part IV turns next to the quantum --- the book's largest part ---
where the count becomes an amplitude and the amplitude, squared, becomes a probability. The doorway Brownian
