...
is the proper time a clock counts along its own path. Seven effects carry the chapter, and one of them, the
relativity of frames, un-folds into two: the relativity of simultaneity and the maximality of proper time, two
readings distinct enough to take a section each. The count, which through four parts was carried, balanced, and
...
A frame is a local labeling of refinement events, and no labeling is privileged. The fact that fixes this is
the speed of light: Maxwell's equations give one speed $c$ for light, the same in every frame, and a constant
...
accumulates --- but the interval combining the two is the same for every observer.
...
each other as the speed rises, while holding the interval fixed, with $\gamma$ the factor by which clocks slow
...
preferred frame because there is no preferred labeling. What every observer agrees on is the interval, the
count of steps along a history; the frames differ only in how they coordinate it, and the Lorentz
...
any shift as the whole is slowly rotated. If light moved through an ether, the arm aligned with the Earth's
...
equal-content paths regardless of direction: $\tau^2$ is unchanged when the spatial axes turn.

The honest floor is a finite count obligation --- the count of steps along a path is isotropic, the same in
every direction. The reading is that the null result proves the metric isotropic. There is no ether to drag the
light because there is no preferred direction in the count; equal-content paths cost equal interval, and the
...
The first of Einstein's two readings is that simultaneity is not absolute. Look again at the boost,
\begin{equation}
...
sloping slice of spacetime for another. There is no operational procedure that forces two observers in relative
...
is relative. The boost mixes time and space, so two observers slice ``now'' differently, and there is no fact of
the matter, beyond each frame's own labeling, about which of two distant events came first.

\section{The Einstein Effect, II: proper time is maximal}
\label{se:einstein-proper-time}

The second reading is about which path between two events a clock actually takes. Among all the admissible
...
\begin{equation}
  \tau = \int d\tau,
\end{equation}
the longest interval, the most distinguishable steps a clock can count between them. This is the counterpart,
...
--- so the stay-at-home twin, not the traveler, has aged more. The twin who travels the maximal-proper-time
...
counts the most --- the twin on the straight, maximal path the one who ages most.
...
\end{equation}
and the observer recording more events is the one resolving the finer order. The honest floor is a finite count
...
has the finer clock and resolves the finer causal order; relative motion is not so much a thing moving through
space as two records resolving the same history to different depths.
...
\end{equation}
each tick removing one. The honest floor is a finite count obligation --- time is the count of resolved degrees
of freedom, one eliminated per tick. The reading is that the arrow of time is counted resolution. Each tick
closes an open degree of freedom and cannot reopen it, so the count only rises; the asymmetry of time is the
irreversibility of resolving the underdetermined, the arrow pointing the way the count grows.
...
inertial labeling can reconcile, registering the acceleration as extra refinements in the record. The clock
that accumulates the denser causal record --- more ticks, more resolved events --- has the larger proper time
and ages more, its merged count the base count plus the refinements the acceleration forced. The effect has
...
return times, and the twin with the denser record is the one who has aged more --- proper time the count,
dilation its smooth shadow.

\section{The Sagnac Effect: rotation as a proved loop residue}
\label{se:sagnac}

The Sagnac effect is the third time the book closes a real loop and proves its residue. Send two light beams
...
approaching one and takes less, so the two return with a phase difference. For a ring of area $A$ rotating at
angular velocity $\Omega$ the time difference is
\begin{equation}
  \Delta t = \frac{4\Omega A}{c^2},
\end{equation}
the Sagnac shift, proportional to the enclosed area and the rotation rate. Read through the holonomy, this is
...
times the book closed a real loop and proved its residue --- the core is proved: the signed tally difference
between the co- and counter-rotating legs is a proved nonzero loop residue, a finite theorem about the loop, not
merely an analogy.

The honest floor is a smooth-shadow analogy with a proved core --- the continuum $\Delta t = 4\Omega A/c^2$ is
the smooth shadow, but the nonzero signed residue beneath it is proved, the third such proof in the book. The
reading is that rotation is a loop residue. A rotating frame cannot synchronize its clocks because the loop
...
rotation is a proved loop residue --- the third in the book. Part V turns next to gravity proper --- the
