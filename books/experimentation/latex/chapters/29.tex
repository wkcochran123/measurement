...
This is the book's densest chapter and the one closest to the machine. Run the apparatus as a computer and it
...
where finite precision defeats it. Eight effects make the chapter, and one of them, the precision limit von
...
\end{equation}
the round-trip an identity --- nothing lost in flattening a structure to a line and rebuilding it. A single
...
the finite model claiming only the name. The reading is that universality is serialization, not magic. A record
...
and where that gate returns nothing, the refinement stops. A halted ledger is not incomplete; it is complete in
the only sense the order allows, having reached a configuration with no admissible continuation. This inverts
the usual reading of the halting problem: there, that one cannot in general foretell whether a machine halts is
...
The honest floor is a no-go --- when no successor satisfies the order, the process stops, and the stopping is a
wall, not a choice. The reading is that halting is completion, not failure. The machine does not always run on;
where the order admits no next step it must stop, and that stopping is forced by the structure, not selected by
the machine --- a wall the refinement reaches and cannot pass.

\section{The Jupyter Effect: a collision halts the kernel}
\label{se:jupyter}

Asynchronous writes can collide, and a collision halts the kernel. When two updates target the same slot at the
same point in the order but carry different values --- same slot, same order, different values --- there is no
admissible reconciliation, and the kernel does not resolve the conflict but halts. The future cannot be
...
The honest floor is a no-go --- a collision admits no continuation and stops the kernel. The reading is that
...
A physical law can be read as the survivor of an interactive proof. Modern proof theory casts verification as a
...
challenge can break. It is not posited and then defended; it is whatever is left standing when every objection
the record can mount has been answered. The surprising strength of the picture is that the verifier need not be
as clever as the prover: by posing challenges it chooses at random, a modest verifier can catch a cheating
...
rounds, a survivor counted against every admissible objection. The reading is that a law is what survives
...
rule can shorten them: to compress $\Omega$ would be to decide what cannot be decided. Two records can agree on
...
mathematics it is, with the finite model claiming only the name. The reading is that a precise record can still
...
be such a question for set theory, and its undecidability, established in two halves, is the standing instance
of that general wall.

The hypothesis resisted proof for sixty years, and the reason came in two halves. G\"odel showed, in 1940, that
...
and its resolution is a chosen completion rather than a derived fact. The reading is that completion is a
...
dominates, once the precision a step requires exceeds $\epsilon_{\text{mach}}$, further operations do not refine
the answer; they only stir the noise. How fast that error grows is measured by the condition number,
\begin{equation}
  \kappa,
\end{equation}
...
blocked, and the machine-epsilon floor is a wall. The reading is that finite precision is a floor with teeth. A
...
proper. A pseudospectrum bulging into the unstable region warns of growth the eigenvalues conceal.

The honest floor is a no-go --- a computation whose pseudospectrum reaches into the unstable region does not
survive finite precision, however safe its spectrum looks. The reading is that stability is survivorship, not
...
admissible refinement: split the problem, solve the parts, combine.

The honest floor is a finite count obligation --- the recursion is a count of admissible steps, $O(N\log N)$
for the transform and a handful of corrections for the root. The reading is that recursion is refinement that
...
With recursion, the densest chapter closes, and the machine's one power and four walls stand clear. It can
...
its last chapter, and the book toward its end.
