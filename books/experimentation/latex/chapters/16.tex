...
energy one quantum at a time. The first of these is the book's second proved loop residue, and as with the
echo maze of Part II the finite model proves rather than shadows --- it is the place this chapter foregrounds,
because a proof is a stronger thing than an analogy, and the instrument has only a handful of them.
...
longer a hypothesis but a reading on a detector. De Broglie's relation gives its wavelength,
\begin{equation}
  \lambda = \frac{h}{p},
\end{equation}
...
There is a reading of the relation in the count's own terms. A wavelength is a phase that advances a full turn,
$2\pi$, over one wave; $\lambda = h/p$ says the phase a particle carries advances one turn for every $h$ of
...
met. Turn the crystal and the lattice rotates under the fixed sphere; each time a point crosses the surface, a
peak flares. The diffraction pattern is the reciprocal lattice itself, sampled wherever a sphere the size of
the electron's own wavevector can reach it.

Read as the book reads it, this is a closed loop. The electron's path into the crystal and out again, when the
Laue condition holds, encircles a circuit around which the wave's phase accumulates an exact whole number of
cycles --- the phase closes. A closed phase loop carries a residue, and here the residue is $\pm 1$: at a Bragg
...
--- the second place in the book, after the echo maze, where the floor is a theorem rather than a shadow.
...
two-fold of matter running, quite literally, in the family. And the proved residue this chapter foregrounds
...
proved handful the book keeps returning to.
...
angle. The shift is largest for a backward scatter ($\theta = 180^\circ$, $\cos\theta = -1$) and vanishes for a
...
balance. The reading is that Compton scattering is conservation made visible in one event. The photon's
...
Set beside the diffraction that opened the chapter, the Compton shift completes a two-fold. There an electron,
a particle, was read as a wave; here light, a wave, is read as a particle --- one carrying momentum, struck and
recoiling like a ball off a ball. They are not two phenomena but one record read two ways: the wave face and
the particle face of the same matter, and the instrument reads whichever face the experiment is built to see. A
crystal set in the beam reads the wave; a single hard scatter reads the particle. The chapter names the fork
and takes no side --- the record beneath the two readings is single.
...
electron to photon instead of the reverse.
...
the beam. Einstein's reading, in 1905, was that light arrives in quanta of energy $h\nu$, and an electron
absorbs one whole quantum or none --- never a fraction, and never a sum pooled from several. The ejected
...
frequency alone. In the laboratory $K_{\max}$ is read as a voltage: apply a retarding potential to the ejected
...
claims only the name. It records that the effect is named and models none of the emission dynamics. The
reading is that the photoelectric effect is named, the linear law $K_{\max} = h\nu - \Phi$ is the physics, and
...
its totals, and a threshold crossed one quantum at a time. The proof is the chapter's center --- the second
