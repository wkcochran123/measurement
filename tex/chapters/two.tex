...
an instrument to resolve distinctions.  The experimental record is therefore
...
Photons do not require specialized detectors to leave evidence in the
experimental record.  In many cases, their presence is revealed indirectly,
through the structured response of ordinary mechanical or electrical systems.
The key requirement is not sensitivity to individual quanta, but the existence
of a repeatable event that can be correlated across refinements.

A familiar example is provided by the timing light used to adjust a
...
that a coarse clock cannot isolate from its own successive ticks.  The device
...
embedded in a correlant process, may recover structure beyond their nominal
resolution.
...
The significance of the Farmater Effect lies in how bounded measurement reshapes
the experimental record.  When refinement proceeds faster than an instrument can
distinguish, the ledger accumulates a dense chain of fine events whose internal
ordering cannot be resolved.  From the perspective of the instrument, this chain
...
coarse instrument records only their aggregate effect.  The experimental record
...
The dense response is a recursively defined mathematical construction.  It may be
viewed as a map
\[
...
exists at least one admissible history in which all their recorded distinctions
...
introduces a distinction into the experimental history.  Once introduced, that
...
This constraint is independent of physical law.  Even in the absence of a model,
even when predictions fail, recorded distinctions persist.  One may revise
interpretations, introduce new hypotheses, or discover contradictions, but the
fact that a distinction was made remains fixed.  Information, once introduced,
becomes part of the logical substrate of description.

The Euclid Effect thus identifies a minimal informational invariant: object
...
directed acyclic graph $G_p = (V_p, A_p)$ on the set of all admissible
...
for distinctions, not a container for values.  A dense response respects this
economy by identifying all admissible successor events while refusing to encode
anything the record does not force.
...
That is, if an event is recorded as preceding another, then no admissible ledger
...
precedence relation is established in the ledger, it constrains all admissible
comparisons.
\end{axiom}
...
new admissible distinction is present.  As a result, repeated interrogation is
...
refinement that overcomes temporal friction (see Phenomenon~\ref{ph:mott}). 
...
resulting transition is irreversible and final.  As in Phenomenon~\ref{ph:mott}, the
