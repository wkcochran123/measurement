...
processing without violating the one-fact-per-moment principle. Internal
ledgers may grow and be refined within the instrument, but only the final gauge
reading is appended to the experimental record. What is observed is not the raw
sensor interaction, but the result of a structured refinement process that
connects the world to the ledger.

Further, this separation explicitly encodes
...
signal and receives a reflected response before the reading reports the associated symbol.
The emission and reception occur before the display is updated.
...
in place before measurement can proceed at all. 
...
\begin{phenom}{The Kant--Einstein Effect~\cite{einstein1905,kant1781}}
...
comparable. Einstein later demonstrated that even within physics, temporal order
cannot be assumed as absolute. Clocks do not reveal an underlying time; they
...
This interval of silence was identified with unusual clarity in the work of
Peirce~\cite{peirce1878}. Peirce insisted that
observation does not consist in direct access to events, but in the production
of signs after those events have occurred. What reasoning encounters is never
the phenomenon itself, but an inscription that stands in its place. The act of
recording is therefore constitutive of knowledge, not ancillary to it. Between
occurrence and inscription lies an irreducible gap in which interpretation,
computation, and error are possible, and within which no new fact yet exists.



\begin{phenom}{The Peirce Effect~\cite{peirce1878}}
\label{ph:peirce-effect}

\PhStatement
An observation is not the event itself, but the inscription of a distinction
after the event has occurred. Reasoning operates only on records, never on
phenomena directly.

\PhOrigin
Peirce argued that knowledge proceeds through signs rather than immediate
contact with reality. An event in the world gives rise to a sign only after an
act of interpretation or inscription has taken place. The sign is temporally
downstream of the occurrence it represents, and all inference is conducted on
these signs rather than on the raw event itself.

\PhObservation
Every physical instrument exhibits a nonzero interval between interaction and
record. Sensors respond, internal logic executes, and only then is a mark made.
Even when engineered to be arbitrarily short, this interval cannot be removed
without eliminating the act of recording altogether.

\PhConstraint
No admissible history may treat recorded facts as simultaneous with the
phenomena they represent. The ledger necessarily lags the event, and any model
that collapses this distinction conflates observation with occurrence.

\PhConsequence
The fundamental unit of measurement is not an instantaneous event, but the act
of inscription that follows it. The moment of record is therefore an
instrumental moment, not a phenomenal one. Physical law constrains what may be
consistently recorded, not what may transiently occur.
\end{phenom}

Peirce’s account anticipates the \emph{instrumental moment} described here. The
instrument does not register reality continuously, nor does it collapse event
and record into a single instant. It waits, computes, and only then writes. The
ledger speaks only after this process is complete. Prediction belongs to this
silent interval, and measurement begins only when the mark is made. In this
sense, an instrument is precisely the structure that converts events into
records, and the moment of the instrument is the moment at which a distinction
is finally inscribed.

\begin{definition}[Instrumental Moment]
The \emph{instrumental moment} is the moment whose carrier models an instrument’s
refinement process. It is the representational site at which the instrument’s
internal computation resolves and exactly one new distinction is licensed and
recorded in the ledger. The instrumental moment is induced by refinement; it is
not the time of the phenomenon, but the moment at which inscription occurs.
\end{definition}
...
The first act of prediction performed by an instrument is the selection
of its alphabet.  Before any interaction occurs, the instrument commits
to a collection of symbols through which its outcomes will be expressed.
This commitment determines which distinctions the instrument will treat
as meaningful and which variations will be suppressed.  In this sense,
the alphabet embodies the instrument’s initial prediction about how best
to convey the content of its measurements.

An alphabet specifies the set of symbols an instrument is capable of
emitting, and thus the categories into which observed interactions will
be placed.  The measurement itself produces a symbol; the meaning
attributed to that symbol arises from its position within the alphabet.
Prediction enters at this stage not as foresight about future outcomes,
but as the choice of symbolic vocabulary by which outcomes will be
interpreted.

The alphabet is treated as an enumeration rather than as an unordered
set.  Enumeration provides a canonical ordering of symbols that supports
extension and refinement without ambiguity.  As an instrument is
refined, its alphabet may be expanded by subdividing existing symbols,
introducing finer distinctions while preserving the interpretability of
earlier records.

\begin{definition}[Alphabet]
\label{def:alphabet}
An \emph{alphabet} is an enumeration of symbols.  That is, given a type of
symbols $\Sigma$, an alphabet consists of those symbols arranged as an
ordered list in which each symbol occupies a definite position.
\end{definition}

The enumeration fixes the set of distinctions an instrument is capable
of registering and provides a canonical ordering that supports extension
and refinement.  No semantic meaning is attached to the symbols beyond
their role as distinguishable marks.  Meaning arises only through how
symbols participate in records, predictions, and phenomenal laws.

This enumerative structure ensures recoverability.  Refined symbols can
always be related back to their coarser predecessors, allowing records
produced under different stages of refinement to participate in the same
phenomenal laws.  Enumeration is therefore not a mere convenience, but
the structural mechanism by which an instrument’s predictive vocabulary
can evolve without invalidating its prior measurements.

By construction, the first set of moments of an instrument is the enumeration of
its predicted alphabet. Before an instrument can record facts, it must first
establish what distinctions are even admissible. The printing of a dial, the
construction of a character set and display, or the hand drawing of tick marks
for a sundial are not ancillary acts; they are the initial refinements by which
the instrument’s alphabet is made explicit.

These acts take time in the ordinary sense, but their significance is not
temporal. They represent the first phase of refinement, in which no phenomena
are yet measured and no facts are yet recorded. Instead, the instrument commits
to a finite or countable set of symbols that future records may draw from. The
ledger remains empty, but the carrier on which refinement will later occur is
already being shaped.

In this way, the earliest instrumental moments do not correspond to observed
events in the world, but to the construction of the instrument itself. The
alphabet precedes the ledger. Only once the predicted distinctions are fully
enumerated can subsequent moments license the recording of facts. Measurement
begins not with observation, but with the prior decision of what can be said.
...
\subsection{Constituent Parts}
\label{sec:instrument-parts}

An instrument consists of a symbol alphabet, a predictor governing its
expected behavior, and a ledger in which outcomes are recorded.  These
components together determine how the instrument interacts with a
phenomenon and how its responses may be refined.

\subsection{Forward Refinement}
\label{sec:forward-refinement}

Forward refinement extends an existing record by producing additional
symbols.  This corresponds to continued operation of the instrument
under the same construction, generating a longer ledger without
altering past entries.

\subsection{Backward Refinement}
\label{sec:backward-refinement}

Backward refinement revisits prior records with improved resolution.
Such refinement may reinterpret earlier measurements without modifying
the underlying ledger, provided that recoverability is maintained.

\section{The Clock}
\label{sec:clock}

The clock is a distinguished instrument whose sole function is to
produce a regular sequence of records.  Rather than defining time, the
clock provides a reference against which other instruments may be
coordinated.  Temporal structure is thus derived from measurement,
not assumed in advance.

Does not assume CH.

\section{Cantor's Instrument}
\label{sec:cantor-instrument}

Cantor's Instrument represents the limiting case of refinement for a
counting-based measurement device.  By extending a rational instrument
through arbitrarily fine refinement, one approaches a continuous
response without assuming the existence of a completed continuum.

Assumes CH.

\subsection{Dense Response}
\label{sec:dense-response}

The dense response describes the behavior of an instrument under
unbounded refinement.  At this stage, responses may be made arbitrarily
fine, yet remain grounded in rational distinctions.  This construction
captures the maximal refinement achievable without invoking additional
structure.

\subsection{Domain Response}
\label{sec:domain-response}

The domain response emerges when refinement behavior stabilizes under
equivalence.  This response may be represented as a real-valued map,
constructed from refinement limits rather than assumed a priori.  No
appeal to the Continuum Hypothesis is required for this construction.


Not all distinctions introduced during refinement correspond to
recoverable structure.  Representational noise arises when multiple
admissible refinement histories collapse into indistinguishable
records.  Such noise reflects the limits of representation rather than
experimental error, and marks the boundary beyond which further
refinement is inadmissible.

\subsection{Historic Models}

Given a Cantor's instrument, we can define a historic model as some domain
response and use the decodemap of the enumeration naturals to apply to build
a predictor from any physical model of time.  This is a Gauge model and
assumes the CH.

Given an Einstein's instrument, we can define a historic model as a bayesian
predictor.

Proposition: A Cantor's instrument refines to an Einsteins' instrument through
the ceil coarsening map.  The reverse is not true. The Bayesian model is
only guaranteed to match the historic model at the anchor points.

So CH => At least one, possibly two distinct models: a phenomenal model and a bayesian model.  The phenomenal
model may or may not be continuous, it just needs to assume it.  
