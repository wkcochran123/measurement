...
\begin{equation}
e = \{(i, j, k) | i \in I \text{,} j \in \mathbf{N}, j \leq |\Sigma_i|, \text{ and } k\in\mathbb{N}\}.
\end{equation}
...
have produced the logged 3-tuples. Models study mechanisms to
...
There are only a finite number of possible instruments an observer can use.
...
The resulting structure is finite and combinatorial.  It identifies exactly
which rational values are sufficient to encode the domain response of the ledger
under refinement.  This encoding does not depend on sensor geometry, clock rate,
or readout mechanism.  It depends only on the pattern of distinctions that have
been activated.  Rational numbers appear here as a minimal coding basis: a
countable alphabet for approximation, not a commitment to underlying continuity.
...
constraints.  Domain responses are order-respecting in the following sense: for
any two events $e_a, e_b \in E$,
\[
e_a \prec e_b \;\;\Longrightarrow\;\; \forall\, k < k_a, \quad r_b^{(k)} \nprec r_a^{(k)} .
\]

That is, if an event is recorded as preceding another, then no ledger
may reverse that order.  Nor may it be assumed that a distinct instrument has
already recorded an event solely because it has recorded a later one.  Once a
precedence relation is established in the ledger, it constrains all comparisons.
\end{axiom}
...
metric, or numerical structure is present a priori. Instead, they constrain the
...
\item \textbf{Identification of an Admissible Record} $r_b$ exists.
...
to certify a persistent state \emph{above or below} the alarm threshold.
