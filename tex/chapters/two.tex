...
A speedometer does not produce numbers; it produces symbols displayed on the
dashboard of the car. A radar gun does something similar: measurements of
electromagnetic pulses are combined to produce symbols on a readout. Sometimes
those symbols match, as when both displays use the same units and markings.
Sometimes they do not, as when one display reports speed in kilometers per hour
and another uses a different scale. In every case, what is presented is not a
number in the abstract, but a \emph{gauge}.

That gauge may take different forms. An LCD speedometer presents speed as a
finite set of digit strings. An analog dial presents speed as the position of a
needle against a finite collection of tick marks, often with two superimposed
graduations. The form differs, but the constraint is the same: each instrument
admits only a finite collection of distinguishable symbols. That finite
collection, regardless of how it is rendered, is what we call an
\emph{alphabet}.
...
The limitations of the forward Euler
...
in the interior nor rewrite any ordering that has already been recorded.
  ...
  The next chapter therefore proceeds from the repaired ledger forward,
    taking as primitive only the irreversible extension operator
      \texttt{extend} : $\text{Ledger} \to \text{Ledger}$.
        Repeated application of this operator certifies that some invariants can be
          estimated only by expending time to accumulate sufficient refinements, while
            others remain forever silent until the next distinction is logged.
