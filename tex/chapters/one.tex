...
\item the image $\eta(X)$ is finite or locally finite, and
\item for any admissible refinement of the enumeration, the induced map on
addresses is recoverable from the record.
\end{enumerate}
The map $\eta$ is not required to be injective, surjective, or invertible.  Two
enumeration maps related by an order-preserving relabeling of $\mathbb{N}$ are
considered observationally equivalent.
\end{definition}
