...
the instrument that produced it. A truth is what is asserted to hold beyond any
...
emerges from the disciplined management of their unavoidable tension.
...
A \emph{truth}, on the other hand, is a constraint on what may be agreed upon.  
...
abstract, and then fail to keep them separate when familiar dial enters the
...
The needle appears to report a fact directly, yet what is observed is an
instrument translating motion into a symbol according to a convention. The
fact is not the marking on the dial, but the agreement that different observers
would record the same symbol from similar dials under the same conditions.
...
reliable at predicting the future~\ref{ph:truth-effect}.
\end{itemize}
...
This is the point at which Phenomenon~\ref{ph:truth-effect} enters. No finite 
collection of
...
without contradiction. Chapters~\ref{chap:continuuum} and~\ref{chap:dynamics} show how 
continuous structure and
dynamical laws arise as minimal, stable representations of dense refinement,
rather than as primitive assumptions. Chapters~\ref{chap:motion} through~\ref{chap:entropy} 
extend this
...
infinitesimal structure inaccessible to all instruments is already admitting
...
Galileo contribution reframed this constraint positively. By insisting that
...
similar conditions are permitted to enter the record. What survives repetition
...
The next phenomenon captures this inversion. In Proposition~\ref{prop:spline},
we will show this excludes every continuation
that is unsupported by the record, permitting a refinement only when all other
possibilities are ruled out as nonexistent or incompatible.
...
position, and succession rather than through intrinsic identity.  
Alphabets are presented as indexed lists, and ledgers evolve
by appending entries in sequence, among other enumerable concepts presented in this
manuscript.  These operations implicitly assume the
existence of a common interface for addressing enumerable collections.
...
known personal name, Kushim, appears not in narrative or myth, but on an
accounting tablet recording quantities received. Kushim enters history as a
ledger entry, not as a character~\ref{schmandtbesserat1992}. Together, these 
mark the same principle:
existence is not granted by explanation, but by being counted.
