...
but ignores color. A clock measures duration but not distance. What qualifies as
...
concern was not utility, but epistemic license~\cite{berkeley1734}.  Predictive power,
according to Phenomenon~\ref{ph:truth-effect}, is never guaranteed.
...
Phenomenon~\ref{ph:pythagoras} asserts that the inability
...
\begin{phenom}{The Pythagoras--Planck Effect~\ref{euclid300bc,planck1900}}
\label{ph:pythagoras}
...
and~\ref{chap:experimental} formalizes
...
measurement can actually distinguish. Any argument that depends on
infinitessimal structure inaccessible to all instruments is already admitting
information that the experimental ledger cannot contain. Such structure may be
consistent within a theory, but it is not recoverable from fact. Berkeley's
objection is therefore not metaphysical, but operational: distinctions without
records are not merely unknown, they are inadmissible.

Galileo reframed this constraint positively. By insisting that
natural phenomena be described through quantities that can be compared,
reproduced, and communicated, Galileo implicitly bound scientific claims to
what could be stably written down. Measurement replaces immediate sensation
with symbol, but only symbols that different observers can agree upon under
similar conditions. What survives repetition
becomes fact; what cannot be reproduced dissolves into private experience.
...
though they had always been present. Phenomeon~\ref{ph:fact-effect} marks this
...
Truths arise only after facts have been collected, and their role is
...
mathematical constructions: counting.
...
rotations occurred, and in what temporal order. Everything else is derived.
...
Refinement proceeds by changing what is counted. A wheel with finer markings,
an encoder with more teeth, or a sensor that registers partial rotations
introduces a new enumeration with smaller increments. The resulting ledger
contains more entries, ordered more finely, and supports new derived quantities.
What appears as continuous motion is thus revealed as a sequence whose apparent
smoothness depends on the density of its enumeration.
...
personal name associated with measurement that we possess, Kushim, appears not
in narrative or myth, but on an accounting tablet tallying receivables
\cite{schmandtbesserat1992}. Kushim enters history as a mark of responsibility
for a count, not as a character in a story. Together, these mark the same
...
this finite decomposition works. If you could not tally how often the wheel signaled a turn, or
...
Counting enters here not as ornament but as necessity. To count how often a
wheel signals a turn or how often a clock signals a tick is to establish the
only stable interface through which different instruments can later be brought
into agreement. Without tallying, there is no principled way to justify treating
two speed readouts, produced by different devices, as comparable at all.

For this reason, a single reading is never just a bare number. It is an entry in
a growing history. Each entry records which instrument registered the change,
which mark was selected from its alphabet, and its position within the ordered
sequence of prior entries. The meaning of the record depends on this context; the
same mark occurring once or occurring many times does not play the same role.
...
enumerable and fully recorded.  Structures that admit random
...
well as presence, or to discuss how a ledger may extend in time, one must be
...
A \emph{decoding map} is a rule for reading outcomes from a finite enumeration.
Given a state space $S$ and a set of outcomes $X$, a decoding map consists of an
enumeration of elements of $X$ together with a function
\[
\zeta : S \times \mathbb{N} \to X \cup \{\varnothing\},
\]
which returns the outcome recorded at position $n$ when it exists, and
$\varnothing$ otherwise. The decoding map does not assert completeness or
invertibility; it merely specifies how recorded outcomes are to be read from the
enumeration.

The presence of $\varnothing$ records the absence of an entry, not a failure of
the map. A decoding map therefore interprets an enumeration as a partial
history, indexed by count, without presupposing that every index corresponds to
a recorded outcome.
\end{definition}

\section{A Trivial Enumeration}
...
\subsection{Time Series as Enumerations of Ledgers}
\label{subsec:ledger-timeseries}

Up to this point, ledgers have been introduced as inductive objects: finite
records that grow by the justified addition of new entries. This construction
captures the minimal structure required to speak about succession without
presupposing a background notion of time. Nevertheless, the language of
measurement and prediction routinely refers to \emph{time series}: ordered
collections of observations indexed by count. The question, therefore, is not
whether ledgers can be treated as time series, but under what conditions such an
identification is admissible.

The key observation is that a time series carries no structure beyond ordered
succession. When stripped of any assumed continuum or clock parameter, a time
series is simply a partially ordered set of records in which each new element
extends the history formed by its predecessors. In this sense, a time series
does not precede the ledger; it is a way of viewing the ledger once an
enumeration of justified refinements has been fixed.

This relationship can be stated precisely.

\begin{proposition}[Time Series as an Enumeration of Ledgers]
\label{prop:timeseries-ledgers}
Every time series admits a representation as an enumeration of ledgers. That is,
given a time series of records, there exists an admissible enumeration such that
each successive index corresponds to a ledger extending the previous one by
exactly one record.
\end{proposition}

\begin{proofsketch}
By definition, a time series consists of records ordered by justified
succession, with no two distinct records occupying the same position. This
ordering induces a countable structure: records may be addressed by their order
of appearance without introducing additional distinctions. Proposition~1
establishes that such ordered succession always admits a trivial enumeration by
counting.

Fix any admissible enumeration of the records. For each natural index $t$, define
the ledger $\mathcal{L}_t$ to consist of all records whose assigned positions are
at most $t$. The defining property of a time series ensures that
$\mathcal{L}_{t+1}$ extends $\mathcal{L}_t$ by the addition of a single new
record, and that no previously recorded entry is altered or removed. In this
way, the time series is realized as a chain of ledgers indexed by the natural
numbers.

The Lean implementation mirrors this construction directly: existence of an
enumeration is assumed as an admissibility condition, and the associated family
of ledgers is defined by successive extension. No appeal is made to a primitive
notion of time or continuity; the indexing arises solely from the order in which
records are justified.
\end{proofsketch}

This proposition completes the conceptual circle of the chapter. Ledgers
provide the primitive notion of recorded succession, while time series emerge
as a particular representation of that succession once an enumeration has been
chosen. In the next chapter, this relationship is made explicit in the simplest
possible case, where a single ledger enumerated by successor realizes the
operational notion of a clock.
